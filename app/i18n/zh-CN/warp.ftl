...
workspace-left-panel-ssh-manager-menu-connect-split-pane = 在当前标签页分屏连接
...
mcp-credentials-rejected = MCP 服务器拒绝了配置的凭据（HTTP 401）。请检查令牌、有效期及权限。服务器未请求 OAuth 登录。
mcp-header-unresolved-secrets = MCP 请求头“{ $header }”包含未解析的密钥：{ $secrets }。请配置这些密钥后重试。未发送任何请求。
mcp-oauth-unavailable = MCP 服务器需要认证。请启用 MCP OAuth 或配置有效的静态凭据请求头。
mcp-headless-oauth-required = 此 MCP 服务器需要交互式 OAuth，无法在无界面模式下完成。若服务器支持，请配置静态凭据请求头；也可先在同一台设备的 InfiniShell 中完成认证，再重试。
mcp-startup-missing-secrets = 缺少 MCP 密钥：{ $secrets }。请配置这些密钥后再启动此服务器。
...
ai-error-skill-not-available = 当前会话无法使用此技能。请选择可用技能列表中的技能。
ai-error-skill-name-ambiguous = 多个技能使用此名称。请使用可用技能列表中的完整 skill_path。
...
terminal-ssh-enhanced-opt-in-enable-error = InfiniShell 无法为此主机启用增强型 SSH：{ $error }
terminal-ssh-enhanced-opt-in-required = 由于已启用 { $options }，InfiniShell 增强型 SSH 需要为每台主机单独配置 OpenSSH。
terminal-ssh-enhanced-opt-in-security-impact = 这将为此主机禁用自动更新主机密钥和击键时序混淆。
terminal-ssh-enhanced-opt-in-prompt = 按 Enter 或输入 yes 更新 ~/.ssh/config 并继续；输入 no 则使用原生 OpenSSH [Y/n]：
terminal-ssh-enhanced-opt-in-native-fallback = 正在使用原生 OpenSSH；本次会话无法使用 InfiniShell SSH 扩展功能。
terminal-ssh-enhanced-opt-in-enabled = 已为 { $host } 启用增强型 SSH；正在继续本次连接。
terminal-ssh-enhanced-opt-in-invalid-choice = 请按 Enter，或输入 yes 或 no。
...
sftp-file-manager-title = 文件管理
sftp-pane-title = SFTP：{ $path }
sftp-rename-placeholder = 输入新名称
sftp-folder-name-placeholder = 文件夹名称
sftp-search-placeholder = 搜索文件…
sftp-status-connecting = 正在连接…
sftp-status-disconnected = 已断开连接
sftp-status-loading = 正在加载…
...
sftp-delete-description-one = 确定要删除“{ $name }”吗？此操作不可撤销。
sftp-delete-description-many = 确定要删除 { $count } 个项目吗？此操作不可撤销。
sftp-delete-title = 确认删除
sftp-rename-title = 重命名
sftp-rename-current-name = 当前名称：{ $name }
sftp-create-folder-title = 新建文件夹
sftp-file-details-title = 文件详情
...
sftp-detail-type = 类型
sftp-detail-size = 大小
sftp-detail-modified = 修改时间
sftp-detail-permissions = 权限
sftp-detail-path = 路径
sftp-move-description = 将“{ $source }”移动到 { $target }
sftp-move-title = 移动文件
sftp-move-button = 移动
sftp-overwrite-upload-description = 远程文件 { $name } 已存在，是否覆盖？
sftp-overwrite-download-description = 目标文件 { $name } 已存在，是否覆盖？
sftp-overwrite-title = 确认覆盖
sftp-overwrite-button = 覆盖
sftp-close-transfers-title = 关闭传输面板
sftp-close-transfers-description = 有正在进行的传输任务，关闭将中断所有传输并清空记录。确定要关闭吗？
sftp-transfer-pending = 等待中
sftp-transfer-in-progress = 传输中
sftp-transfer-completed = 已完成
sftp-transfer-failed = 失败
sftp-transfer-cancelled = 已取消
sftp-transfer-panel-title = 传输（{ $count }）
sftp-error-connection = 连接错误：{ $message }
sftp-error-operation = 操作错误：{ $message }
sftp-error-local-io = 本地 I/O 错误：{ $message }
sftp-error-no-credentials = 未找到凭据：{ $message }
sftp-error-transfer-cancelled = 传输已取消
sftp-error-create-channel = 创建 SFTP 通道失败：{ $error }
sftp-error-connection-cancelled = 连接已取消
sftp-error-server-config-not-found = 未找到服务器配置
sftp-error-read-server-config = 读取服务器配置失败：{ $error }
sftp-error-not-connected = 未连接到服务器
sftp-error-list-directory = 列出目录失败：{ $error }
sftp-error-delete = 删除失败：{ $error }
sftp-error-invalid-file-name = 文件名包含非法字符
sftp-error-upload = 上传失败：{ $error }
sftp-error-download = 下载失败：{ $error }
sftp-error-name-empty = 名称不能为空
sftp-error-folder-name-empty = 文件夹名称不能为空
sftp-error-name-path-separator = 名称不合法：不能包含路径分隔符
...
sftp-backend-list-directory = 列出目录失败 { $path }：{ $error }
sftp-backend-read-directory-entry = 读取目录条目失败：{ $error }
sftp-backend-read-metadata = 读取元数据失败：{ $error }
sftp-backend-delete-file = 删除文件失败 { $path }：{ $error }
sftp-backend-delete-directory = 递归删除目录失败 { $path }：{ $error }
sftp-backend-create-directory = 创建目录失败 { $path }：{ $error }
sftp-backend-rename = 重命名失败 { $source } → { $target }：{ $error }
sftp-backend-resolve-path = 解析路径失败 { $path }：{ $error }
sftp-backend-stat = 获取文件信息失败 { $path }：{ $error }
sftp-backend-create-local-directory = 创建本地目录失败：{ $error }
sftp-backend-upload-file = 上传文件失败：{ $error }
sftp-backend-open-remote-file = 打开远程文件失败：{ $error }
sftp-backend-create-local-file = 创建本地文件失败：{ $error }
sftp-backend-read = 读取失败：{ $error }
sftp-backend-write = 写入失败：{ $error }
sftp-backend-flush = 刷新失败：{ $error }
sftp-ops-resolve-auth = 解析认证失败：{ $error }
sftp-ops-rename-remote-temporary = 重命名远程临时文件失败：{ $error }。临时文件：{ $path }
sftp-ops-rename-downloaded-temporary = 重命名失败：{ $error }。已下载的临时文件保留在：{ $path }
sftp-ops-read-password = 读取密码失败：{ $error }
sftp-ops-password-not-stored = 服务器 { $host } 未存储密码
sftp-ops-key-path-missing = 已选择密钥认证，但未指定密钥路径
agent-local-child-wasm-unsupported = WASM 构建不支持本地子智能体。
...
toolbar-editor-left-side = 左侧
toolbar-editor-right-side = 右侧

settings-ai-usage-duration-monthly = 每月
settings-ai-usage-limit-description = 这是你账户 AI 额度的{ $duration }上限。
settings-ai-default-prompt-submission-mode = 默认提示词提交方式
settings-ai-default-prompt-submission-description = 智能体仍在响应时提交新提示词的处理方式。你可以在每个对话中使用自动排队开关覆盖此设置。
settings-ai-default-long-running-submission-mode = 长时间运行命令期间的默认提交方式
settings-ai-default-long-running-submission-description = 智能体正在操作其发起的长时间运行命令时，提交提示词的处理方式。排队的提示词会在命令完成后发送给智能体。
settings-ai-submit-rich-input = 使用 Ctrl+Enter 提交富输入
settings-ai-gemini-enterprise-section = Gemini Enterprise
settings-ai-gemini-enterprise-toggle = 使用 Gemini Enterprise 凭证
settings-ai-gemini-enterprise-description = InfiniShell 会通过工作区的 Gemini Enterprise Google Cloud 项目路由符合条件的请求。
settings-ai-gemini-enterprise-description-managed = InfiniShell 会通过工作区的 Gemini Enterprise Google Cloud 项目路由符合条件的请求。此设置由你的组织管理。
settings-ai-custom-routers-section = 自定义路由器
settings-ai-custom-routers-description = 根据任务复杂度或自定义规则，自动将任务路由到指定模型。自定义路由器会显示在模型选择菜单中。
custom-router-summary-default = 默认：
custom-router-summary-easy = 简单：
custom-router-summary-medium = 中等：
custom-router-summary-hard = 困难：
custom-router-summary-rule-count = { $count } 条规则

settings-command-show-agent-tips = 显示智能体提示
settings-command-hide-agent-tips = 隐藏智能体提示
settings-command-show-use-agent-footer = 显示“使用智能体”页脚
settings-command-hide-use-agent-footer = 隐藏“使用智能体”页脚
settings-command-allow-full-access-denylist-bypass = 允许“完全访问”绕过命令拒绝列表
settings-command-require-full-access-denylist-approval = 要求审批“完全访问”中的拒绝列表命令
settings-command-allow-auto-approve-denylist-bypass = 允许自动审批绕过命令拒绝列表
settings-command-require-auto-approve-denylist-approval = 要求审批自动审批模式中的拒绝列表命令
settings-command-show-code-review-tab-button = 在标签栏显示代码评审按钮
settings-command-hide-code-review-tab-button = 在标签栏隐藏代码评审按钮

toggle-suffix-git-operations = 提交与拉取请求生成
toggle-suffix-agent-command-history = 在历史记录中包含智能体执行的命令
toggle-suffix-conversation-history = 工具面板中的会话历史
toggle-suffix-model-picker = 提示词中的模型选择器
toggle-suffix-coding-agent-toolbar = 编码智能体工具栏
toggle-suffix-warp-credit-fallback = InfiniShell 额度回退
toggle-suffix-auto-toggle-rich-input = 根据智能体状态自动显示或隐藏富输入
toggle-suffix-auto-open-rich-input = 编码智能体会话启动时自动打开富输入
toggle-suffix-auto-dismiss-rich-input = 提交提示词后自动关闭富输入
toggle-suffix-window-custom-size = 以自定义尺寸打开新窗口
toggle-suffix-window-blur-texture = 亚克力窗口模糊纹理
toggle-suffix-tools-panel-consistency = 在标签页间保持工具面板可见性一致
toggle-suffix-agent-font-match-terminal = 智能体字体匹配终端字体
toggle-suffix-notebook-font-match-terminal = 笔记本字号匹配终端字号
toggle-suffix-vertical-tabs-restored = 恢复窗口时显示垂直标签页面板
toggle-suffix-latest-prompt-tab-title = 使用最近的用户提示词作为标签页对话标题
toggle-suffix-preserve-active-tab-color = 新标签页保留当前标签页颜色
toggle-suffix-alt-screen-custom-padding = 备用屏幕中的自定义内边距
toggle-suffix-auto-open-code-review = 自动打开代码评审面板
toggle-suffix-code-review-button = 代码评审按钮
toggle-suffix-code-review-diff-stats = 代码评审按钮上的差异统计
toggle-suffix-project-explorer = 项目浏览器
toggle-suffix-global-file-search = 全局文件搜索
toggle-suffix-show-hidden-project-files = 项目浏览器中的隐藏文件
toggle-suffix-autosuggestion-ignore = 自动建议忽略按钮
toggle-suffix-ssh-control-master = 在 InfiniShell SSH 包装器中复用现有 SSH ControlMaster
toggle-suffix-long-running-notifications = 长时间运行命令通知
toggle-suffix-agent-task-notifications = 智能体任务完成通知
toggle-suffix-needs-attention-notifications = 需要注意时的通知
toggle-suffix-notification-sounds = 通知声音
toggle-suffix-in-app-agent-notifications = 应用内智能体通知
toggle-suffix-mouse-reporting = 鼠标事件上报
toggle-suffix-help-block = 新会话中的帮助命令块
toggle-suffix-at-context-menu = 终端模式中的“@”上下文菜单
toggle-suffix-preserve-input-focus = 选择命令块时保留输入焦点
toggle-suffix-global-workflows-command-search = 命令搜索中的全局工作流

settings-code-show-hidden-files = 在项目浏览器中显示隐藏文件
settings-code-show-hidden-files-description = 在项目浏览器中显示点文件和隐藏文件（以 . 开头）。
settings-code-auto-save = 自动保存
settings-code-auto-save-description = 输入时以及编辑器失去焦点时，自动保存 InfiniShell 文本编辑器中的更改。

settings-appearance-category-tools-panel = 工具面板
settings-appearance-font-default-option = { $font }（默认）
settings-appearance-font-weight-normal = 常规
settings-appearance-font-weight-bold = 粗体
settings-appearance-show-in-dock = 在程序坞中显示 InfiniShell
settings-appearance-tab-hide-title-bar-search-label = 在垂直标签页布局中隐藏搜索框
settings-appearance-tab-hide-title-bar-search-description = 使用垂直标签页布局时，隐藏标题栏中的搜索框。仍可通过命令面板和键盘快捷键使用搜索。
settings-appearance-icon-aurora = 极光
...
settings-features-asynchronous-find = 异步查找
settings-features-asynchronous-find-description = 使用改进的查找实现，使界面在大型输出中搜索匹配项时保持响应。

settings-warp-drive-local-description = InfiniShell Drive 是终端中的本地工作区，可在此设备上保存工作流、笔记本、提示词和环境变量。
settings-warpify-reuse-control-master = 复用现有 SSH ControlMaster
