...
// Zap:BYOP 修复记录测试直接构造 multi-agent api 消息。
...
fn persisted_agent_conversation_for_test(
    ...
    AgentConversation {
        conversation: AgentConversationRecord {
            ...
            last_modified_at,
            // 该 helper 模拟"summary 列写入前的旧行",读侧应回退到从 tasks 推导。
            summary: None,
        },
...
fn test_initialize_historical_conversations_uses_summary_column_without_tasks() {
    ...
    App::test((), |app| async move {
        ...
        history_model.read(&app, |model, _| {
            ...
            // 我方字段名为 `is_restorable_locally`(上游叫 `has_local_data`),语义相同。
            assert!(metadata.is_restorable_locally);
...
fn prompt_history_candidates_seeds_from_snapshot_then_appends_session_prompts() {
    App::test((), |mut app| async move {
        // `write_updated_conversation_state` 会读 GeneralSettings / AppExecutionMode /
        // GlobalResourceHandlesProvider,这些都由该 setup 注册。
...
/// Helper function to create [`AIConversationMetadata`] for testing.
///
/// Zap 的会话元数据是纯本地结构(没有云端 ServerMetadata/权限),所以测试直接构造字面量。
fn create_conversation_metadata(
    ...
    AIConversationMetadata {
        ...
        last_modified_at: Utc::now().naive_utc(),
        initial_working_directory: None,
        credits_spent: Some(credits_spent),
        server_conversation_token: Some(ServerConversationToken::new(server_token.to_string())),
...
fn test_ambient_agent_conversations_excluded_from_list_but_accessible_by_id() {
    App::test((), |mut app| async move {
        ...
        history_model.update(&mut app, |model, _| {
            let regular_metadata = create_conversation_metadata(
                regular_id,
            ...
            let ambient_metadata = create_conversation_metadata(
                ambient_id,
...
fn test_fork_then_bind_handoff_token_updates_cached_metadata_and_emits_refresh_events() {
    ...
    App::test((), |mut app| async move {
        ...
        history_model.read(&app, |model, _| {
            ...
            // 上游的 `has_cloud_data` 派生自 `server_conversation_token.is_some()`,
            // 我方 `AIConversationMetadata` 没有这个冗余字段——上面的断言已完全覆盖。
        });
...
fn statuses_after_stream_error(
    ...
    App::test((), |mut app| async move {
        initialize_history_persistence_for_tests(&mut app);
        // Completing a request with an error reads auth state, so the
        // provider has to be registered. Zap 没有云端 telemetry context provider。
