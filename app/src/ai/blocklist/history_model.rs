...
pub use conversation_loader::{
    CLIAgentConversation, LoadedConversationData,
...
impl BlocklistAIHistoryModel {
    ...
    ///
    /// Zap 的会话历史是纯本地存储,没有云端会话元数据缓存,因此只更新已加载到内存的会话。
    pub fn set_server_metadata_for_conversation(
        ...
        // Emit event so listeners can refresh.
        ctx.emit(BlocklistAIHistoryEvent::UpdatedConversationMetadata {
            terminal_surface_id,
            conversation_id,
        });
    ...
    /// terminal surface that currently holds it.
    ///
    /// Use this when the user **explicitly navigates** to a conversation in a
    /// different terminal surface (e.g. from the conversation history or command
    /// palette). For automatic follow-ups during tool-call cycles, use
    /// [`Self::mark_active_conversation_id`] instead — it updates the active
    /// pointer without touching other terminal surfaces.
    ...
    /// `ConversationServerTokenAssigned` so restored child-agent links can resolve.
    ...
    pub fn fork_conversation_at_exchange(
        ...
        //
        // `source_tasks` 保留截断前的完整任务列表,供 BYOP 修复 sidecar 迁移使用。
    ...
    pub fn delete_conversation(
        ...
        // conversations have no terminal_surface_id; handlers that filter on it
    ...
    ///
    /// A conversation can be shared if we have server metadata available on the
    /// loaded conversation.
    ...
    /// Returns the server conversation metadata for a loaded conversation.
    ///
    /// Zap 的会话历史是纯本地存储,没有云端会话元数据缓存,因此只查内存中已加载的会话。
...
fn merged_remote_child_placeholder_conversation_data(
    ...
    AgentConversationData {
        ...
        // Zap 本地 sidecar(BYOP 压缩 / 修复状态、CLI subagent block 快照):
        // remote child placeholder 与云端会话都不产生这些数据,合并结果留空。
        compaction_state_json: None,
        byop_repair_state_json: None,
        cli_subagent_block_snapshots_json: None,

        // 项目绑定是纯本地数据,以 placeholder 为准(云端会话不携带)。
...
pub enum BlocklistAIHistoryEvent {
    ...
    /// `terminal_surface_id` is `None` when deleting a historical-only conversation that
    /// is not currently bound to any terminal surface (e.g. right-click delete from the
    ...
    LocalSharedSessionEstablished {
        conversation_id: AIConversationId,
        // 上游 `session_sharing_protocol` crate 已下线,共享会话协议类型改由
        // 本地 `terminal::shared_session::protocol` 提供。
        session_id: crate::terminal::shared_session::protocol::SessionId,
    },
}
