    ...
    App::test((), |mut app| async move {
        ...
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.spawned_uuids.len(),
                1,
                "Managed InfiniShell MCP config should auto-spawn regardless of toggle"
            );
        });
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Managed InfiniShell MCP config should never be despawned by toggle changes, got: {:?}",
                e.despawned_uuids
            );
        });
    });
    ...
    App::test((), |mut app| async move {
        ...
        events.update(&mut app, |e, _| {
            assert!(
                ...
                "Global non-InfiniShell server must not auto-spawn while toggle is off, got: {:?}",
                e.spawned_uuids
            );
        });
        ...
        events.update(&mut app, |e, _| {
            assert_eq!(
                ...
                "Global non-InfiniShell server should spawn when toggle flips on"
            );
        });
        ...
        events.update(&mut app, |e, _| {
            assert_eq!(
                e.despawned_uuids,
                vec![installation_uuid],
                "Global non-InfiniShell server should despawn when toggle flips off"
            );
        });
    });
}
