...
impl SkillManager {
    ...
    /// 与 `list_skill_inventory` 的优先级保持一致。Bundled skill 不进
    /// `skills_by_name` 索引,这里回落到本地 bundled 目录按 name 兜底。
    ...
    /// 汇总所有文件型 skill,按 name 分组供 Skill 管理面板展示。
    /// 每组内按 provider 优先级 → scope → 路径排序,第一项即生效的默认实现。
    pub fn list_skill_inventory(&self, ctx: &AppContext) -> Vec<SkillInventoryItem> {
        ...
        for skill in self.skills_by_path.values() {
            // 面板只处理本地路径的 skill;远端 host 发布的 skill 不进清单。
