...
pub use self::handoff_compose::{HandoffComposeState, HandoffComposeStateEvent, HandoffEntryPoint};
...
use crate::terminal::shared_session::protocol::{AgentAttachment, ParticipantId};
...
const AGENT_MODE_AI_DISABLED_AUTODETECTION_DISABLED_HINT_KEY: &str =
    "terminal-input-run-commands-hint";

// Rotating hint text options for new Agent Mode conversations
const AGENT_MODE_HINT_KEYS: &[&str] = &[
    "terminal-input-agent-hint-deploy-react-vercel",
    "terminal-input-agent-hint-debug-python-ci",
    "terminal-input-agent-hint-setup-microservice",
    "terminal-input-agent-hint-fix-node-memory-leak",
    "terminal-input-agent-hint-backup-postgres",
    "terminal-input-agent-hint-migrate-mysql-postgres",
    "terminal-input-agent-hint-monitor-aws",
    "terminal-input-agent-hint-build-fastapi",
    "terminal-input-agent-hint-optimize-sql",
    "terminal-input-agent-hint-github-actions",
    "terminal-input-agent-hint-redis-cache",
    "terminal-input-agent-hint-kubernetes-pods",
    "terminal-input-agent-hint-bigquery-pipeline",
    "terminal-input-agent-hint-ssl-https",
    "terminal-input-agent-hint-refactor-legacy-code",
    "terminal-input-agent-hint-unit-tests",
    "terminal-input-agent-hint-elk-logs",
    "terminal-input-agent-hint-oauth-express",
    "terminal-input-agent-hint-optimize-docker",
    "terminal-input-agent-hint-ab-testing",
];
...
fn get_stable_agent_mode_hint_key(cached_hint: &mut Option<&'static str>) -> &'static str {
    if let Some(key) = cached_hint {
        key
    } else {
        let new_key = get_agent_mode_new_conversation_hint_key();
        *cached_hint = Some(new_key);
        new_key
    }
}

fn translate_input_key(key: &'static str) -> String {
    crate::i18n::t_or(key, key)
}
    ...
    pub fn placeholder_text(&self) -> Option<String> {
        match self {
            InputSuggestionsMode::UserQueryMenu {
                ...
                ..
            } => Some(crate::t!("terminal-input-search-queries")),
            InputSuggestionsMode::UserQueryMenu {
                action: UserQueryMenuAction::Rewind,
                ..
            } => Some(crate::t!("terminal-input-search-queries-rewind")),
            InputSuggestionsMode::ConversationMenu => {
                Some(crate::t!("terminal-input-search-conversations"))
            }
            InputSuggestionsMode::SkillMenu => Some(crate::t!("terminal-input-search-skills")),
            InputSuggestionsMode::ModelSelector => Some(crate::t!("terminal-input-search-models")),
            InputSuggestionsMode::ProfileSelector => {
                Some(crate::t!("terminal-input-search-profiles"))
            }
            InputSuggestionsMode::SlashCommands if FeatureFlag::AgentView.is_enabled() => {
                Some(crate::t!("terminal-input-search-commands"))
            }
            InputSuggestionsMode::PromptsMenu => Some(crate::t!("terminal-input-search-prompts")),
            InputSuggestionsMode::IndexedReposMenu => {
                Some(crate::t!("terminal-input-search-indexed-repos"))
            }
            InputSuggestionsMode::PlanMenu { .. } => Some(crate::t!("terminal-input-search-plans")),
            _ => None,
        }
    }
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings(vec![
        ...
        FixedBinding::custom(
            ...
            InputAction::Up,
            crate::t!("keybinding-desc-terminal-show-history"),
    ...
    app.register_editable_bindings([EditableBinding::new(
        ...
        BindingDescription::new(crate::t!("keybinding-desc-input-edit-prompt"))
            .with_custom_description(
                bindings::MAC_MENUS_CONTEXT,
                crate::t!("keybinding-desc-input-edit-prompt"),
            ),
        ...
        && !FeatureFlag::ForceClassicCompletions.is_enabled()
    {
        app.register_editable_bindings([EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-toggle-classic-completions"),
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-command-search"),
            WorkspaceAction::ShowCommandSearch(Default::default()),
        )
        ...
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-history-search"),
        ...
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-open-completions-menu"),
    ...
    if let Some(custom_action) = workflows::CategoriesView::custom_action() {
        app.register_editable_bindings([EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-workflows"),
    ...
    app.register_editable_bindings([
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-open-ai-command-suggestions"),
        ...
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-new-agent-conversation"),
        ...
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-trigger-auto-detection"),
        ...
        EditableBinding::new(
            ...
            crate::t!("keybinding-desc-input-clear-and-reset-ai-context-menu-query"),
...
impl Input {
    ...
    pub(crate) fn attach_ambient_agent_view_model(
        ...
        self.agent_input_footer.update(ctx, |footer, ctx| {
            footer.set_ambient_agent_view_model(footer_model, ctx);
        });
        ...
        // 我方字段名是 `is_dummy_ambient_agent_session`(上游叫 dummy cloud mode session)。
        let is_cloud_mode_composer = self.model.lock().is_dummy_ambient_agent_session();
    ...
    pub(crate) fn new(
        ...
        let agent_input_footer = ctx.add_typed_action_view(|ctx| {
            AgentInputFooter::new(
                ...
                None,
                // 上游这里还传 `handoff_compose_state`(local→cloud 交接 chip),
                // Zap 已删除 handoff 交接 UI,footer 不再需要该参数。
                current_prompt.clone(),
            ...
            AIQueryRouting::NewCloudVm { task_id } => {
                if FeatureFlag::HandoffCloudCloud.is_enabled() {
                    ...
                    if !pending_attachments.is_empty() {
                        log::warn!(
                            "Cloud follow-up attachments are not supported in InfiniShell; sending prompt only"
                        );
                    }
    ...
    fn execute_skill_command(
        ...
        // 上游用它把已排队 skill 的发送路由回排队时所在的对话(并解析该行的附件),
        // 我方 `send_queued_slash_command_request` 不接受该参数,故保留形参占位但不使用。
        _conversation_id_override: Option<AIConversationId>,
        ctx: &mut ViewContext<Self>,
    ) -> bool {
        ...
        let Some(conversation) = history
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                let toast = DismissibleToast::default(crate::t!(
                    "terminal-no-active-conversation-to-export"
                ));
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        match (
            ...
            (InputType::Shell, false) => {
                translate_input_key(AGENT_MODE_AI_DISABLED_AUTODETECTION_DISABLED_HINT_KEY)
            }
            (InputType::Shell, true) => {
                // Ensure hint text is cached for new conversations
                translate_input_key(get_stable_agent_mode_hint_key(
                    &mut self.cached_agent_mode_hint_key,
                ))
            }
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "InfiniShell anything"
                    ...
                    .selected_conversation_status_for_hint(app)
                {
                    Some(status) if status.is_in_progress() => {
                        if is_queue_next_prompt_enabled {
                            ...
                            translate_input_key(AGENT_MODE_AI_ENABLED_STEER_HINT_KEY_CLASSIC)
                        }
                    }
                    Some(_) => {
                        if is_udi_enabled {
                            translate_input_key(AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_KEY_UDI)
                        } else {
                            translate_input_key(AGENT_MODE_AI_ENABLED_FOLLOW_UP_HINT_KEY_CLASSIC)
                        }
                    }
                    None => {
                        // Ensure hint text is cached for new conversations
    ...
    fn cli_agent_rich_input_hint_text(&self, ctx: &ViewContext<Self>) -> Cow<'static, str> {
        ...
        CLIAgentSessionsModel::as_ref(ctx)
            ...
            .map(|session| match session.agent {
                CLIAgent::Unknown => Cow::Owned(translate_input_key(CLI_AGENT_RICH_INPUT_HINT_KEY)),
                _ => Cow::Owned(crate::t!(
                    "terminal-input-enter-prompt-for-agent",
                    agent = session.agent.display_name()
                )),
            })
            .unwrap_or_else(|| Cow::Owned(translate_input_key(CLI_AGENT_RICH_INPUT_HINT_KEY)))
    }
            ...
            workflows::CategoriesViewEvent::WorkflowSelected {
                ...
                send_telemetry_from_ctx!(
                    TelemetryEvent::WorkflowSelected(WorkflowTelemetryMetadata {
                        ...
                        // `cloud_object_models` 与 app 各有一份同名的
                        // `GenericStringObjectId`,经 String 中转转换。
                        enum_ids: workflow
                            .as_workflow()
                            .get_server_enum_ids()
                            .into_iter()
                            .map(|id| String::from(id).into())
                            .collect()
                    }),
                    ctx
                );
    ...
    fn enter_ai_mode_for_ai_context_menu_selection(&mut self, ctx: &mut ViewContext<Self>) {
        if AISettings::as_ref(ctx).is_any_ai_enabled(ctx) {
            // 上游给 `ensure_agent_mode_for_ai_features` 新增了 decision_source 参数。
            // @ 上下文菜单挑选附件/上下文属于"附件强制进入 AI 模式"。
            self.ensure_agent_mode_for_ai_features(
                false,
                Some(InputTypeAutoDetectionSource::AttachmentForcedAi),
                ctx,
    ...
    pub(crate) fn submit_queued_prompt(
        ...
        if let Some(argument) = compact_and_argument {
            ...
            // 先发摘要请求,再把后续 prompt 连同原附件排回该对话的队列。
            ...
            if let Some(initial_prompt) = argument.filter(|prompt| !prompt.trim().is_empty()) {
                QueuedQueryModel::handle(ctx).update(ctx, |model, ctx| {
                    model.append(
                        conversation_id,
        ...
        // `execute_slash_command` 的处理保持一致:这里不传 `query_id`。
        self.ai_controller.update(ctx, move |controller, ctx| {
    ...
    pub(crate) fn submit_queued_prompt_for_active_pane(
        ...
        // 因此该分支整体删除,直接落到下面的共享会话观看端 / 本地 Agent Mode 路径。
        ...
        if self.model.lock().shared_session_status().is_viewer() {
            // 我方 `ServerConversationToken` 是 String newtype(上游是 Uuid 包装,
            // 所以那边要 `parse().map(ServerConversationToken::from_uuid)` 往返一次),
            // 这里直接取用即可。
    ...
    fn submit_viewer_ai_query(&mut self, ctx: &mut ViewContext<Self>) -> bool {
        ...
        let selected_conv_id = self
            ...
            .as_ref(ctx)
            .selected_conversation_id(ctx);
        let server_conversation_token = selected_conv_id.and_then(|id| {
            BlocklistAIHistoryModel::as_ref(ctx)
                .conversation(&id)
                .and_then(|conv| conv.server_conversation_token().cloned())
        });
        ...
        let attachments: Vec<AgentAttachment> = self
            ...
            .filter_map(|context| match context {
                // `warp_terminal::BlockId` 与 shared-session protocol 的 `BlockId`
                // 是两个同名的 String newtype,经 String 中转转换。
                AIAgentContext::Block(block) => Some(AgentAttachment::BlockReference {
                    block_id: String::from(block.id).into(),
                }),
    ...
    /// 提交输入缓冲区时上报一次 `InputBufferSubmitted`。
    /// (合并时该 helper 连同上游的其它输入遥测一起丢失,这里按 `app/src/server/telemetry.rs`
    /// 现存的事件形状恢复。)
                ...
                Some(selected_workflow_state) => {
                    send_telemetry_from_ctx!(
                        TelemetryEvent::WorkflowExecuted(WorkflowTelemetryMetadata {
                            ...
                            // 同上:两份同名 `GenericStringObjectId`,经 String 中转。
        ...
        match action {
            InputAction::FocusInputBox => {
                ActionAccessibilityContent::Custom(AccessibilityContent::new(
                    translate_input_key(INPUT_A11Y_LABEL_KEY),
                    // TODO (a11y) use bindings from user settings
                    translate_input_key(INPUT_A11Y_HELPER_KEY),
