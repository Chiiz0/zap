...
pub(super) struct State {
    ...
    /// correctly. Notably, Zsh does NOT support this well in bracketed paste mode (which we use for all Zap
...
impl ansi::Handler for GridHandler {
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // 追加被大小上限拒绝时，单元格宽度不会改变。
            if !self.grid[row][col].push_zerowidth(c, /* 记录超长字素警告 */ true) {
                return;
            }
    ...
    fn identify_terminal<W: std::io::Write>(&mut self, writer: &mut W, intermediate: Option<char>) {
        match intermediate {
            ...
            Some('>') => {
                ...
                // Zap, rather than passing the higher `Pv` value, we decided to use one of the
    ...
    fn report_xtversion<W: std::io::Write>(&mut self, writer: &mut W) {
        ...
        let _ = writer.write_all(format!("\x1bP>|InfiniShell({version})\x1b\\").as_bytes());
    }
