...
fn test_find_url_omits_trailing_periods() {
    ...
    let blockgrid = mock_blockgrid("Visit https://github.com/zerx-lab/warp/issues.");
    assert_eq!(
        blockgrid
            .grid_handler
            .url_at_point(Point { row: 0, col: 10 }),
...
fn emoji_variation_selector_drops_width_promotion_when_line_wrap_is_disabled() {
    ...
    let original_cell = grid.grid_storage()[VisibleRow(0)][3].clone();

    grid.input('\u{FE0F}');

    assert_eq!(grid.grid_storage()[VisibleRow(0)][3], original_cell);
...
fn wide_character_spacers_inherit_the_base_cell_hyperlink() {
    ...
    input_hyperlinked(&mut grid, "https://promoted.example.com", "⚠");

    grid.input('\u{FE0F}');

    let storage = grid.grid_storage();
    let wide_hyperlink_id = storage[VisibleRow(0)][0].hyperlink_id().unwrap();
    assert_eq!(
        storage[VisibleRow(0)][1].hyperlink_id(),
        Some(wide_hyperlink_id)
    );
    let promoted_hyperlink_id = storage[VisibleRow(0)][2].hyperlink_id().unwrap();
...
fn leading_wide_character_spacer_is_not_hyperlinked() {
    ...
    let hyperlink_id = storage[VisibleRow(1)][0].hyperlink_id().unwrap();
